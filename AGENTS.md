...
**Use pnpm.** Never run `npm install` or `yarn` in this project — always `pnpm install`.

```bash
pnpm add --save-exact <package>@<version>       # pin exact, no ^ or ~
pnpm add --save-exact -D <package>@<version>    # dev dep
pnpm audit --audit-level=moderate               # run before adding any new dep
```

One-time machine config (run once per dev):
```bash
...
Before adding a dep:
- Check weekly downloads and last publish date on npmjs.com.
- Read its `dependencies` and `peerDependencies` — every transitive package is attack surface.
- Avoid packages with postinstall scripts unless every line is reviewed.
- After adding, run `pnpm audit --audit-level=moderate` and resolve any findings.
