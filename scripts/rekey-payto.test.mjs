...
// test and claim.test.mjs's identical one could otherwise race on the same
// file. An exclusive lock directory (mkdir is atomic) serializes them.
...
// module's own import chain (claim.mjs -> rekey-payto.mjs).
test('importing rekey-payto.mjs does not load the root .env or mutate process.env', async () => {
    ...
    try {
      const result = spawnSync(
        ...
        [
          '--input-type=module',
          '-e',
          `import(${JSON.stringify(rekeyPayToModuleHref)}).then(() => ` +
      ...
      assert.equal(
        result.stdout.trim(),
        '',
        `importing rekey-payto.mjs must never load the root .env (leaked: ${JSON.stringify(result.stdout)})`,
      )
    } finally {
