...
```bash
cd contracts
npm install -D vitest @algorandfoundation/algorand-typescript-testing @vitest/coverage-v8
```
...
This script runs via `npm run deploy` and handles the full TestNet setup. It reads account mnemonics from `.env` (set by genaccounts.ts + dotenv).
...
```bash
cd contracts && npm run deploy:ci
```
...
```bash
cd mcp && echo '{"method":"initialize","params":{},"id":1,"jsonrpc":"2.0"}' | npm run dev 2>/dev/null | head -5
```
